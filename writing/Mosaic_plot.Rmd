...
mosaic_fueltype <- ggplot(data = altfuel_larger) +
  geom_mosaic(aes(x = product(fuel_type_code, facil_group), fill = fuel_type_code)) +   
  labs(y="Fuel Types Used", x="Facility Type", title = "Mosaic Plot of Fuel Type by Facility Type") +
  scale_y_discrete(guide=guide_axis(n.dodge=3))

mosaic_fueltype + theme(axis.text.x = element_text(angle = 90)) + theme(
